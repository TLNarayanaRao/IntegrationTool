...
function GroupEditor({ group, groups, resources, activities, task, tasks, schemas, properties, customFunctions, onClose, onSave, onDelete }: any) {
  ...
  return <div className="modal-backdrop"><div className="runtime-modal group-editor-dialog"><header><span><Workflow/><span><b>Executable Group</b><small>Nested canvas and runtime execution boundary</small></span></span><button onClick={onClose}>×</button></header><main>
    ...
    {!["scope", "none", "critical_section", "transaction_jdbc"].includes(draft.type) && <section className="group-data-mapper"><DataSourcePane properties={properties} sources={dataSources} customFunctions={customFunctions} runtimeVariables={[...["iterate", "for_each"].includes(draft.type) ? [{ name: draft.config.itemVariable || "item", label: "Current item", type: "any" }] : [], ...["while", "repeat", "repeat_on_error", "iterate", "for_each"].includes(draft.type) ? [{ name: draft.config.indexVariable || "index", label: "Iteration index", type: "integer" }] : [], ...(draft.type === "repeat_on_error" ? [{ name: "error", label: "Current group error", type: "object" }] : [])]}/><div className="group-condition-targets"><header><DataNodeIcon/><span><b>Group data and condition</b><small>Drag data, functions, properties, or constants into the runtime field.</small></span></header>
      ...
      {["iterate", "for_each"].includes(draft.type) && <GroupExpressionTarget label="Collection to iterate" kind="collection" value={draft.config.source} onChange={(value: any) => patchConfig("source", value)} placeholder="${activities.read.output.records}" help="Resolved at group entry. The group executes exactly once for each collection item."/>}
    ...
    {["iterate", "for_each"].includes(draft.type) && <label>Item variable<input value={draft.config.itemVariable || "item"} onChange={(event) => patchConfig("itemVariable", event.target.value)}/></label>}
    {draft.type === "repeat" && <label>Safety iteration limit<input type="number" min="1" value={draft.config.maxIterations ?? 10000} onChange={(event) => patchConfig("maxIterations", Number(event.target.value))}/></label>}
    {draft.type === "repeat_on_error" && <><label>Maximum retry count<input type="number" min="0" value={draft.config.retryCount ?? 3} onChange={(event) => patchConfig("retryCount", Number(event.target.value))}/></label><label>Retry interval (seconds)<input type="number" min="0" value={draft.config.retryIntervalSeconds ?? 0} onChange={(event) => patchConfig("retryIntervalSeconds", Number(event.target.value))}/></label></>}
