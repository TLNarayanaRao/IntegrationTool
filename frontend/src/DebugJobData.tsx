...
export default function DebugJobData({ state, tasks, onClose }: any) {
  ...
  const format = (value: any) => value === undefined ? "(not captured)" : typeof value === "string" ? value : JSON.stringify(value, null, 2);
    ...
    <div className="runtime-modal debug-job-data-dialog">
      ...
      <main>
        ...
        <section className="debug-job-payloads">
          ...
          {selected ? <div className="debug-job-columns"><article><header>INPUT</header><pre>{format(input)}</pre></article><article><header>OUTPUT</header><pre>{format(selected.output)}</pre></article></div> : <div className="debug-job-empty">No data captured for this selection yet. Continue or step through the flow.</div>}
        </section>
      </main>
