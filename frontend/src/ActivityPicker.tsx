...
const icon = (asset?: string) => asset ? <img src={`/activity-icons/${asset}.png`} alt="" /> : null;
...
export default function ActivityPicker({ menu, packs, addActivity, close }: any) {
  ...
  const results = useMemo(() => {
    ...
    return term ? flatten(groups).filter((entry) => `${entry.label} ${entry.path} ${entry.operation || ""}`.toLowerCase().includes(term)) : [];
  }, [groups, query]);
  ...
  const add = (entry: Entry) => { addActivity(entry, { x: menu.cx, y: menu.cy }); close(); };
  const openEntry = (entry: Entry) => entry.children?.length ? setTrail((items) => [...items, entry]) : add(entry);
  return <div className="activity-picker simple-picker" style={{ left, top, width, height }} onClick={(event) => event.stopPropagation()}>
    <header><Search/><input autoFocus aria-label="Search activities" placeholder="Search activities…" value={query} onChange={(event) => setQuery(event.target.value)}/>{query && <button aria-label="Clear search" onClick={() => setQuery("")}><X/></button>}<button aria-label="Close activity picker" onClick={close}><X/></button></header>
    {query ? <div className="activity-search-results simple-search"><small>{results.length} MATCHES</small><div className="picker-card-grid">{results.map((entry, index) => <button key={`${entry.path}-${entry.label}-${index}`} onClick={() => add(entry)}>{icon(entry.asset)}<span><b>{entry.label}</b><small>{entry.path}</small></span></button>)}</div>{!results.length && <p>No activity matches “{query}”.</p>}</div> : <div className="picker-browser">
      ...
      <section><header>{trail.length > 1 && <button aria-label="Back one activity level" onClick={() => setTrail((items) => items.slice(0, -1))}><ChevronLeft/></button>}<span><b>{current?.label || "Activities"}</b><small>{trail.map((entry) => entry.label).join(" / ")}</small></span></header><div className="picker-card-grid">{entries.map((entry, index) => <button key={`${entry.label}-${index}`} onClick={() => openEntry(entry)}>{icon(entry.asset)}<span><b>{entry.label}</b><small>{entry.operation || (entry.children?.length ? `${flatten(entry.children).length} activities` : entry.type)}</small></span>{entry.children?.length ? <ChevronRight/> : null}</button>)}</div></section>
    </div>}
  </div>;
}
