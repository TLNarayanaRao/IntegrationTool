...
export default function ActivityPicker({ menu, packs, addActivity, addGroup, close }: any) {
  ...
  return <div className="activity-picker simple-picker" style={{ left, top, width, height }} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
    ...
    {query ? <div className="activity-search-results simple-search"><small>{results.length} MATCHES</small><div className="picker-card-grid">{results.map((entry, index) => <button className={!entry.asset ? "picker-card-text-only" : undefined} key={`${entry.path}-${entry.label}-${index}`} onClick={() => add(entry)}>{icon(entry.asset)}<span><b>{entry.label}</b><small>{entry.path}</small></span></button>)}</div>{!results.length && <p>No activity matches “{query}”.</p>}</div> : <div className="picker-browser">
