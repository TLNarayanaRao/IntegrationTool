...
import { ArrowLeft, ArrowRight, Braces, CheckCircle2, CodeXml, Columns3, Copy, Edit3, FileJson, FileText, FolderOpen, Save, Search, Undo2, WrapText, X } from "lucide-react";
import "./file-utilities.css";
import "./file-utilities-layout.css";

export type UtilityMode = "xml" | "json" | "compare";
type FileReference = { id: string; name: string; size: number; modified?: number; file?: File };
...
function Viewer({ mode }: { mode: "xml" | "json" }) {
  ...
  const closeRef = useRef(reader.close), activeReference = useRef<FileReference | undefined>(undefined); activeReference.current = view?.reference;
  ...
  const open = async () => { const next = await reader.choose(mode); if (next) { reader.close(view?.reference); await load(next); } };
  ...
  const save = async () => { if (!view || !dirty) return; setBusy(true); try { const replacement = view.binary ? bytesFromHexLines(displayed) : displayed, reference = await reader.save(view.reference, view.offset, view.bytes.length, replacement); const next = await reader.read(reference, view.offset, pageSize); setView(next); setDraft(null); setDecodeAngles(false); setMessage(window.fabricDesktop ? "Changes saved safely to the file." : "Edited file downloaded by the browser."); } catch (error: any) { setMessage(`Save failed: ${error.message}`); } finally { setBusy(false); } };
  const highlighted = query ? displayed.split("\n").filter((line) => line.toLowerCase().includes(query.toLowerCase())) : [];
  return <section className="utility-viewer"><div className="utility-toolbar"><button className="primary" onClick={open}><FolderOpen/> Open {mode.toUpperCase()}</button><button disabled={!view || view.binary || busy} onClick={pretty}><Braces/> Pretty print</button><button disabled={!dirty || busy} onClick={() => void save()}><Save/> Save</button><button disabled={!dirty || busy} onClick={() => { setDraft(null); setDecodeAngles(false); setMessage("Unsaved changes discarded."); }}><Undo2/> Revert</button><button className={wrapLines ? "active" : ""} aria-pressed={wrapLines} onClick={() => setWrapLines((current) => !current)}><WrapText/> Wrap lines</button>{mode === "xml" && <label><input type="checkbox" checked={decodeAngles} disabled={!view?.text} onChange={(event) => { setDecodeAngles(event.target.checked); if (draft !== null && event.target.checked) setDraft(decodeAngleEntities(draft)); }}/> Decode &amp;lt; and &amp;gt;</label>}<label className="utility-search"><Search/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find in current window…"/></label></div>{view ? <><header className="utility-file-heading"><span>{mode === "xml" ? <CodeXml/> : <FileJson/>}<b>{view.reference.name}</b><small>{formatBytes(view.reference.size)} · editable {view.binary ? "binary/hex" : "UTF-8 text"}</small></span><i>{dirty ? "UNSAVED" : view.reference.size > pageSize ? "WINDOWED" : "COMPLETE"}</i></header><textarea className={`utility-code${wrapLines ? " wrap" : ""}`} spellCheck={false} wrap={wrapLines ? "soft" : "off"} value={displayed} onChange={(event) => setDraft(event.target.value)}/><FilePager value={view} pageSize={pageSize} setPageSize={setPageSize} locked={dirty} navigate={(offset) => void load(view.reference, offset)}/>{query && <div className="utility-findings">{highlighted.length} matching lines in this window</div>}</> : <div className="utility-empty"><FileText/><b>No {mode.toUpperCase()} file open</b><span>Files are read in bounded windows so very large payloads do not become a single in-memory editor value.</span></div>}<footer className="utility-message">{busy ? "Working…" : message}{dirty ? " · Unsaved changes" : ""}</footer></section>;
}
...
function CompareViewer() {
  ...
  return <section className="utility-viewer compare-viewer">
    <div className="utility-toolbar compare-toolbar"><button className="primary" onClick={() => void open("left")}><FolderOpen/> Select left</button><button className="primary" onClick={() => void open("right")}><FolderOpen/> Select right</button><button disabled={!leftDirty || busy} onClick={() => void saveSide("left")}><Save/> Save left</button><button disabled={!rightDirty || busy} onClick={() => void saveSide("right")}><Save/> Save right</button><button className={editMode ? "active" : ""} disabled={!left || !right} onClick={() => setEditMode((value) => !value)}><Edit3/> {editMode ? "Show differences" : "Edit files"}</button><label><input type="checkbox" checked={differencesOnly} onChange={(event) => setDifferencesOnly(event.target.checked)}/> Differences only</label></div>
    ...
    <div className="compare-headings"><span><b>{left?.reference.name || "Left file"}</b><small>{left ? `${formatBytes(left.reference.size)}${leftDirty ? " · UNSAVED" : ""}` : "Select any file type"}</small></span><span><b>{right?.reference.name || "Right file"}</b><small>{right ? `${formatBytes(right.reference.size)}${rightDirty ? " · UNSAVED" : ""}` : "Select any file type"}</small></span></div>
    {left && right ? editMode ? <div className="compare-editors"><textarea aria-label="Edit left file window" spellCheck={false} value={leftText} onChange={(event) => setLeftDraft(event.target.value)}/><textarea aria-label="Edit right file window" spellCheck={false} value={rightText} onChange={(event) => setRightDraft(event.target.value)}/></div> : <div className="compare-grid" role="table">{rows.map((row) => <React.Fragment key={`${row.leftNumber || "x"}-${row.rightNumber || "x"}-${row.index}`}><label className={`diff-select ${row.kind}`}><input type="checkbox" aria-label={`Select difference row ${row.index + 1}`} disabled={row.kind === "same" || !compatible} checked={selected.has(row.index)} onChange={(event) => setSelected((current) => { const next = new Set(current); if (event.target.checked) next.add(row.index); else next.delete(row.index); return next; })}/></label><div className={`${row.kind} line-number`}>{row.leftNumber || ""}</div><pre className={row.kind}>{row.left || " "}</pre><div className={`${row.kind} line-number`}>{row.rightNumber || ""}</div><pre className={row.kind}>{row.right || " "}</pre></React.Fragment>)}</div> : <div className="utility-empty"><Columns3/><b>Side-by-side file comparison</b><span>Select two files, choose individual differences, copy them in either direction, edit either window, and save each synchronized file.</span></div>}
    <div className="utility-pager"><button disabled={dirty || !left || !right || !offset} onClick={() => void navigate(Math.max(0, offset - pageSize))}><ArrowLeft/> Previous</button><span>{left && right ? `${formatBytes(offset)} window${dirty ? " · save or revert before navigating" : ""}` : "No comparison loaded"}</span><select disabled={dirty} value={pageSize} onChange={(event) => setPageSize(Math.min(MAX_WINDOW, Number(event.target.value)))}><option value={262144}>256 KB window</option><option value={1048576}>1 MB window</option><option value={4194304}>4 MB window</option></select><button disabled={dirty || !left || !right || eof} onClick={() => void navigate(offset + pageSize)}>Next <ArrowRight/></button></div>
