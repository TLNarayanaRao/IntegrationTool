...
export default function MapperStudio({ config, schemas = [], onClose, onSave }: any) {
  ...
  const test = async () => {
    try {
      const response = await fetch("/api/mapper/test", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input: JSON.parse(inputText), mappings: rules, targetSchema: schemaObject(targetText), options: config }) });
