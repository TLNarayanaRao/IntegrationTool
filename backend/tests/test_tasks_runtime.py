...
class TaskRuntimeTests(unittest.TestCase):
    ...
    def test_subtask_messaging_export_import_and_debug(self):
        ...
        self.assertIn('filename="Task-Runtime-Test.mpackage"', exported.headers['content-disposition'])
        with zipfile.ZipFile(io.BytesIO(exported.content)) as archive:
            self.assertIn('tasks/child.json', archive.namelist())
            self.assertIn('resources/kafka/k1.json', archive.namelist())
        imported = self.client.post('/api/projects/import', files={'file':('project.mpackage', exported.content, 'application/zip')})
        self.assertEqual(imported.status_code, 200)
        self.assertEqual(len(imported.json()['tasks']), 2)
        for legacy_name in ('project.ifproject', 'project.ifpackage'):
            legacy = self.client.post('/api/projects/import', files={'file':(legacy_name, exported.content, 'application/zip')})
        ...
        mina_package = self.client.get('/api/projects/task-runtime-test/package?target=cloud&environment=production')
        self.assertEqual(mina_package.status_code, 200)
        self.assertIn('filename="task-runtime-test-1.0.0-cloud.mpkg"', mina_package.headers['content-disposition'])
        ...
        self.assertIn('filename="task-runtime-test-1.0.0-cloud.ifpkg"', cloud_package.headers['content-disposition'])
    ...
    def test_python_archive_contains_direct_async_source_without_fabric_descriptors(self):
        ...
        self.assertEqual(response.status_code, 200, response.text)
        self.assertIn('filename="task-runtime-test-1.0.0-on-prem.pympkg"', response.headers['content-disposition'])
