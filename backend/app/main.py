...
def _lifecycle_logs(item: Project, endpoints: list[dict]) -> list[dict]:
    now = log_timestamp()
    logs = [
        {'time': now, 'level': 'INFO', 'kind': 'lifecycle', 'message': f'MINA runtime initializing application {item.name}', 'startedAt': now},
...
def export_project(project_id: str):
    ...
    with zipfile.ZipFile(stream, 'w', zipfile.ZIP_DEFLATED) as archive:
        archive.writestr('manifest.json', json.dumps({'format':'mina-project','version':2,'projectId':item.id,'name':item.name,'features':{'groups':'runtime-v1'}}, indent=2))
    ...
    stream.seek(0)
    filename = re.sub(r'[^A-Za-z0-9_.-]+','-',item.name).strip('-') or item.id
    return StreamingResponse(stream, media_type='application/zip', headers={'Content-Disposition':f'attachment; filename="{filename}.mpackage"'})
...
def python_source_files(project_payload: dict) -> dict[str, bytes]:
    """Generate readable workflow Python using the packaged Fabric DSL."""
    header = '# Generated by MINA Studio. Changes are overwritten on export.\n\n'
    def snake(name: str) -> str:
        return re.sub(r'(?<!^)(?=[A-Z])', '_', str(name)).replace('-', '_')
    def expression(value: object, indent: int = 0) -> str:
        if isinstance(value, str) and value.startswith('${') and value.endswith('}'):
            return f'ref({value[2:-1]!r})'
        if isinstance(value, dict):
            if not value: return 'options()'
            pad = ' ' * indent; inner = ' ' * (indent + 4)
            parts = [f'{inner}{snake(key)}={expression(child, indent + 4)},' if re.fullmatch(r'[A-Za-z_]\w*', snake(key)) else f'{inner}({key!r}, {expression(child, indent + 4)}),' for key, child in value.items()]
            return 'options(\n' + '\n'.join(parts) + '\n' + pad + ')'
        if isinstance(value, list): return '[' + ', '.join(expression(item, indent) for item in value) + ']'
        return repr(value)
    def named(values: dict, indent: int = 8) -> str:
        return ''.join(f"{' ' * indent}{snake(key)}={expression(value, indent)},\n" for key, value in values.items())
    dsl = '''"""Small runtime DSL used by generated MINA Python applications."""
from app.models import Activity, CustomFunction, EnvironmentProperty, GroupDefinition, Project, SchemaAsset, SharedResource, TaskDefinition, Transition
import re
...
def _normalize(value):
    if isinstance(value, dict): return {_camel(str(key)): _normalize(child) for key, child in value.items()}
    if isinstance(value, list): return [_normalize(child) for child in value]
    return value

def options(*pairs, **values):
    result = dict(pairs); result.update(values); return _normalize(result)

def ref(path): return '${' + str(path) + '}'
def point(x=100, y=100): return {'x': x, 'y': y}

class ActivityRef:
    def __init__(self, task, activity): self.task, self.activity = task, activity
    @property
    def id(self): return self.activity.id
    def on_success(self, target, label=''):
        self.task._transition(self, target, 'success', label); return target
    def when(self, condition, target, label=''):
        self.task._transition(self, target, 'success_condition', label, condition); return target
    def otherwise(self, target, label=''):
        self.task._transition(self, target, 'success_no_match', label); return target
    def on_error(self, target, label=''):
        self.task._transition(self, target, 'error', label); return target

class ActivityFamily:
    def __init__(self, task, kind): self.task, self.kind = task, kind
    def __getattr__(self, operation):
        return lambda **values: self.task._activity(self.kind, operation if operation != 'default' else '', **values)

class Task:
    def __init__(self, id, name, kind='starter', description='', input_schema=None, output_schema=None):
        self.id, self.name, self.kind, self.description = id, name, kind, description
        self.input_schema, self.output_schema = input_schema or {}, output_schema or {}
        self.activities, self.transitions, self.groups = [], [], []
    def __getattr__(self, kind): return ActivityFamily(self, kind)
    def _activity(self, kind, operation, **values):
        activity = Activity(id=values.pop('id'), type=kind, name=values.pop('name'), position=values.pop('position', point()), config=options(operation=operation, **values))
        self.activities.append(activity); return ActivityRef(self, activity)
    def _transition(self, source, target, kind, label='', condition=''):
        self.transitions.append(Transition(id=f'{source.id}-to-{target.id}-{len(self.transitions)+1}', source=source.id, target=target.id, type=kind, label=label, condition=condition))
    def group(self, **values):
        values = dict(values); values['config'] = options(**values.pop('config', {}))
        self.groups.append(GroupDefinition(**values))
    def build(self): return TaskDefinition(id=self.id, name=self.name, kind=self.kind, description=self.description, input_schema=self.input_schema, output_schema=self.output_schema, activities=self.activities, transitions=self.transitions, groups=self.groups)

def resource(kind, *, id, name, **config): return SharedResource(id=id, type=kind, name=name, config=options(**config))
def schema(*, id, name, content): return SchemaAsset(id=id, name=name, content=content)
def function(*, id, name, parameters=None, expression='', description=''): return CustomFunction(id=id, name=name, parameters=parameters or [], expression=expression, description=description)
def profile(name, *values):
    return name, [EnvironmentProperty(key=value['key'], value=value.get('value', ''), data_type=value.get('dataType', value.get('data_type', 'string'))) for value in values]
def project(*, resources, schemas, tasks, profiles, custom_functions=None, **values): return Project(resources=resources, schemas=schemas, tasks=tasks, properties=dict(profiles), custom_functions=custom_functions or [], **values)
'''
    files: dict[str, bytes] = {
        'application/python/__init__.py': b'"""Generated MINA Python application."""\n',
        'application/python/fabric_dsl.py': dsl.encode('utf-8'),
        'application/python/tasks/__init__.py': b'"""Generated executable task modules."""\n',
        'application/python/README.md': b'''# MINA Python archive

This archive contains generated workflow Python. Tasks use named activity calls
such as `task.sap.idoc_listener(...)` and transitions such as
`listener.on_success(parser)`. The included `fabric_dsl.py` converts this
readable source into the MINA runtime model at startup.
''',
    }
    task_imports: list[str] = []; task_builders: list[str] = []
    for index, task in enumerate(project_payload.get('tasks', [])):
        module_name = f'task_{index}_{re.sub(r"[^A-Za-z0-9_]+", "_", str(task.get("id") or "task")).strip("_") or "task"}'
        builder_name = f'build_task_{index}'
        task_imports.append(f'from .tasks.{module_name} import build_task as {builder_name}')
        task_builders.append(f'        {builder_name}(),')
        activity_variables: dict[str, str] = {}; used: set[str] = set(); body: list[str] = []
        for position, activity in enumerate(task.get('activities', [])):
            base = snake(activity.get('name') or activity.get('id') or f'activity_{position}').lower()
            variable = re.sub(r'\W+', '_', base).strip('_') or f'activity_{position}'
            while variable in used: variable += '_next'
            used.add(variable); activity_variables[str(activity.get('id'))] = variable
            config = dict(activity.get('config') or {}); operation = str(config.pop('operation', '') or 'default')
            values = {'id': activity.get('id'), 'name': activity.get('name'), 'position': activity.get('position', {})}
            values.update(config)
            body.append(f'    {variable} = task.{activity.get("type")}.{operation}(\n{named(values)}    )\n')
        for transition in task.get('transitions', []):
            source, target = activity_variables.get(str(transition.get('source'))), activity_variables.get(str(transition.get('target')))
            if not source or not target: continue
            kind = transition.get('type') or 'success'; label = transition.get('label') or ''; condition = transition.get('condition') or ''
            if kind == 'success_condition': body.append(f'    {source}.when({expression(condition)}, {target}, label={label!r})\n')
            elif kind == 'success_no_match': body.append(f'    {source}.otherwise({target}, label={label!r})\n')
            elif kind == 'error': body.append(f'    {source}.on_error({target}, label={label!r})\n')
            else: body.append(f'    {source}.on_success({target}, label={label!r})\n')
        for group in task.get('groups', []): body.append(f'    task.group(\n{named(group)}    )\n')
        fields = {key: value for key, value in task.items() if key not in {'activities', 'transitions', 'groups'}}
        files[f'application/python/tasks/{module_name}.py'] = (header +
            'from ..fabric_dsl import Task, options, point, ref\n\n\n'
            'def build_task():\n'
            '    task = Task(\n' + named(fields) + '    )\n' + ''.join(body) + '    return task.build()\n').encode('utf-8')
    resources = ''.join(f"        resource({resource.get('type')!r},\n{named({**{key: value for key, value in resource.items() if key not in {'type', 'config'}}, **dict(resource.get('config') or {})}, 12)}        ),\n" for resource in project_payload.get('resources', []))
    files['application/python/resources.py'] = (header + 'from .fabric_dsl import options, ref, resource\n\n\ndef build_resources():\n    return [\n' + resources + '    ]\n').encode('utf-8')
    schemas = ''.join(f"        schema(\n{named(schema_value, 12)}        ),\n" for schema_value in project_payload.get('schemas', []))
    files['application/python/schemas.py'] = (header + 'from .fabric_dsl import options, ref, schema\n\n\ndef build_schemas():\n    return [\n' + schemas + '    ]\n').encode('utf-8')
    project_fields = {key: value for key, value in project_payload.items() if key not in {'resources', 'schemas', 'tasks', 'custom_functions', 'properties'}}
    functions = ''.join(f"        function(\n{named(function_value, 12)}        ),\n" for function_value in project_payload.get('custom_functions', []))
    profiles = ''.join(f"        profile({environment!r},\n" + ''.join(f"            options(\n{named(value, 16)}            ),\n" for value in values) + '        ),\n' for environment, values in project_payload.get('properties', {}).items())
    files['application/python/project.py'] = (header +
        'from .fabric_dsl import function, options, profile, project, ref\nfrom .resources import build_resources\nfrom .schemas import build_schemas\n' + '\n'.join(task_imports) + '\n\n\n'
        'def build_project():\n    return project(\n' + named(project_fields) +
...
def deployment_package_files(item: Project, target: str, environment: str, artifacts: set[str] | None = None) -> dict[str, bytes]:
    ...
    if str(item.packaging.get('sourceFormat') or '').lower() == 'python':
        manifest['runtime'] = 'integration-fabric-python-source'
        manifest['pythonSource'] = {'entrypoint': 'application/python/project.py', 'formatVersion': 1}
        files['manifest.json'] = json.dumps(manifest, indent=2).encode()
        files.update(python_source_files(sanitized_project))
...
'''.encode()
        ...
        windows_install_root = str(item.packaging.get('windowsInstallRoot') or f'C:\\ProgramData\\MINA\\apps\\{artifact}')
...
'''.encode()
        if 'readme' in selected_artifacts:
            files['deployment/on-prem/README.txt'] = f'''MINA on-premises deployment
...
2. Run install.sh, or import application.json through MINA Control Plane.
...
def build_deployment_archive(item: Project, target: str, environments: list[str], starter_ids: list[str] | None,
    ...
    elif archive in {'mpkg', 'ifpkg', 'ear', 'zip', 'python', 'python-engine', 'python-direct'}:
        with zipfile.ZipFile(stream, 'w', zipfile.ZIP_DEFLATED) as bundle:
            for name, body in files.items(): bundle.writestr(name, body)
        extension = 'ear' if archive == 'ear' else ('zip' if archive == 'zip' else ('pympkg' if archive in {'python', 'python-engine', 'python-direct'} else archive))
        media = 'application/java-archive' if archive == 'ear' else 'application/zip'
    else:
        raise HTTPException(400, 'Archive must be mpkg, zip, ear, tar.gz, python-engine, or python-direct (legacy ifpkg is also accepted)')
...
def package_project(project_id: str, target: str = 'on-prem', environment: str = 'local', environments: str = '', starters: str = '', archive: str = 'mpkg', artifacts: str = ''):
    item = get_project(project_id)
    if not item: raise HTTPException(404, 'Project not found')
    ...
    try:
        ...
        def test_path(value: str) -> str:
            ...
            if path.startswith('last.'):
                return path[5:]
            # Studio activity-name expressions omit the internal activity ID
            # and output envelope: ${IDoc-Parser.IDOC.EDI_DC40.DOCNUM}.
            if '.' in path and not path.startswith(('properties.', 'vars.', 'context.', 'tasks.')):
